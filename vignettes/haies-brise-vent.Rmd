...
# Créer une ligne de haie à l'ouest (simulée par une multiligne)
coords_haie <- rbind(c(-5, 30), c(-5, 170))
haie_ouest <- sf::st_linestring(coords_haie)
haie_ouest <- sf::st_sfc(haie_ouest, crs = 32618)

coords_haie2 <- rbind(c(50, 100), c(150, 100))
haie_interieure <- sf::st_linestring(coords_haie2)
haie_interieure <- sf::st_sfc(haie_interieure, crs = 32618)

haies <- sf::st_sf(
  id = 1:2,
  geometry = sf::st_sfc(haie_ouest, haie_interieure, crs = 32618)
)

# Convertir les haies en POINTS pour les fonctions de distance
# (chaque haie devient une série de points)
set.seed(42)
points_haie <- lapply(1:2, function(i) {
  g <- haies$geometry[[i]]
  if (inherits(g, "sfg_LINESTRING")) {
    ...
    data.frame(
      id = i,
      x = pts[, 1] + rnorm(nrow(pts), 0, 1),
      y = pts[, 2] + rnorm(nrow(pts), 0, 1)
    )
  }
})
...
ggplot(df_vitesse, aes(x = x, y = y, fill = vitesse)) +
  ...
  scale_fill_gradientn(colors = c("darkblue", "lightblue", "white", "orange", "red"),
                        name = "Vitesse (m/s)") +
  geom_sf(data = haies, col = "black", lwd = 1) +
  coord_fixed() +
  theme_minimal() +
  labs(title = "Vitesse du vent estimée",
       subtitle = "5 m/s en entrée, protection des haies",
       x = "X (m)", y = "Y (m)")
```
...
**Note**: La fonction `obtenir_rose_vents()` nécessite une connexion internet pour accéder aux données NASA POWER.
