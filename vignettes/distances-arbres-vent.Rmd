...
# Visualiser
df_vitesse <- as.data.frame(vitesse_simple$vitesse, xy = TRUE)
names(df_vitesse)[3] <- "vitesse"
...
vitesse_dir <- simuler_vitesse_vent_simple(
  dist_result = dist_dir,
  vitesse_ref = 5,
  coef_protection = 0.5
)

df_vitesse_dir <- as.data.frame(vitesse_dir$vitesse, xy = TRUE)
names(df_vitesse_dir)[3] <- "vitesse"

ggplot(df_vitesse_dir, aes(x = x, y = y, fill = vitesse)) +
  geom_raster() +
  scale_fill_gradientn(colors = c("darkblue", "lightblue", "white", "orange", "red"),
                        name = "Vitesse (m/s)") +
  coord_fixed() +
  theme_minimal() +
  labs(title = "Vitesse du vent (amont/aval)",
       subtitle = "Effet différentiel amont vs aval",
       x = "X (m)", y = "Y (m)")
```
